#!/usr/bin/env python3
"""
Compare bilateral peripheral SRPK1-associated and miR-139-5p-associated cortical MSN maps using spatial null models.
...
# x = SRPK1-related cortical MSN map
# y = miR-139-5p-related cortical MSN map
# =========================
...
out_dir = PROJECT_ROOT / "msn_results_cort/a5_periph_srpk1_vs_mirna_v6"
out_dir.mkdir(parents=True, exist_ok=True)

result_file = out_dir / "a5_srpk1_vs_mir1395p_matched_batch_models_burt_results_v6.csv"
meta_file = out_dir / "a5_srpk1_vs_mir1395p_matched_batch_models_meta_v6.json"
roi_order_check_file = out_dir / "a5_dk308_roi_order_check_v6.csv"
...
all_results = []
meta = {
    "analysis": "cortical_A5_SRPK1_MSN_tmap_vs_miR1395p_MSN_tmap",
    ...
    "note": (
        ...
        "The analysis includes the original batch_main and batch_plus_med_history pairs, "
        "plus two SRPK1 neutrophil-adjusted sensitivity comparisons requested in v6. "
...
for model_name, paths in matched_files.items():
    ...
    if len(df) != expected_n_roi:
        raise ValueError(f"Expected {expected_n_roi} ROIs after merge for {model_name}, got {len(df)}")
    ...
    merged_file = out_dir / f"a5_input_merged_maps_{model_name}_aligned_v6.csv"
    null_corr_file = out_dir / f"a5_null_distribution_{model_name}_aligned_v6.csv"
    ...
    all_results.append({
        "comparison": "Peripheral_SRPK1_MSN_tmap_vs_miR1395p_MSN_tmap",
        "model": model_name,
