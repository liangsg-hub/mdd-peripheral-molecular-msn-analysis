#!/usr/bin/env python3
"""
Compare left-hemisphere peripheral SRPK1-associated and miR-139-5p-associated cortical MSN maps using spatial null models.
...
# =========================
# matched x and y maps
# x = SRPK1-related cortical MSN map, left hemisphere only
# y = miR-139-5p-related cortical MSN map, left hemisphere only
# =========================
...
matched_files = {
    "batch_main": {
        ...
        "y_file": mirna_dir / "mir1395p_imaging_map_main.csv",
    },
    "batch_plus_med_history": {
        ...
        "y_file": mirna_dir / "mir1395p_imaging_map_plus_med_history.csv",
    },
...
out_dir = PROJECT_ROOT / "msn_results_cort/a5_periph_srpk1_vs_mirna_lh_v2"
out_dir.mkdir(parents=True, exist_ok=True)

result_file = out_dir / "a5_srpk1_vs_mir1395p_matched_batch_models_burt_results_lh_v2.csv"
meta_file = out_dir / "a5_srpk1_vs_mir1395p_matched_batch_models_meta_lh_v2.json"
roi_order_check_file = out_dir / "a5_dk308_lh_roi_order_check_v2.csv"
...
def prepare_map(path, value_name, label):
    check_file(path, label)

    df = pd.read_csv(path)
    df.columns = [str(x).strip() for x in df.columns]

    missing_cols = sorted(set([roi_col, t_col]) - set(df.columns))
    if missing_cols:
        raise ValueError(f"Missing columns in {label}: {missing_cols}")

    df = df[[roi_col, t_col]].rename(columns={t_col: value_name}).copy()
...
# =========================
# load ROI order and LH Burt nulls
# =========================
...
all_results = []
meta = {
    "analysis": "cortical_A5_SRPK1_MSN_tmap_vs_miR1395p_MSN_tmap_LH",
    ...
    "note": (
        ...
        "share the same ROI order. This v2 script includes matched main and plus_med_history "
        "models, plus two SRPK1 neutrophil-adjusted sensitivity comparisons against the "
        "corresponding miR-139-5p main or plus_med_history maps. SRPK1 batch_main "
        "corresponds to y ~ SRPK1 + Batch + age + sex + EDL + eTIV."
    ),
}
...
for model_name, paths in matched_files.items():
    ...
    srpk1_df_raw = prepare_map(x_file, "srpk1_t", "SRPK1 map")
    mirna_df_raw = prepare_map(y_file, "mir1395p_t", "miR-139-5p map")
    ...
    if len(df) != expected_n_roi:
        raise ValueError(
            f"Expected {expected_n_roi} LH ROIs after merge for {model_name}, got {len(df)}."
        )
    ...
    if len(df) != expected_n_roi:
        raise ValueError(
            f"ROI count after dropna is not {expected_n_roi} for {model_name}."
        )
    ...
    merged_file = out_dir / f"a5_input_merged_maps_{model_name}_lh_v2.csv"
    null_corr_file = out_dir / f"a5_null_distribution_{model_name}_lh_v2.csv"
