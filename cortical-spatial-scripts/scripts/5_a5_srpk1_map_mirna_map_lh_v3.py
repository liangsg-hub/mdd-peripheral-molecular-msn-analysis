#!/usr/bin/env python3
"""
Compare miR-139-5p-associated left-cortical MSN t maps with peripheral
SRPK1-associated left-cortical MSN t maps using Map 1-specific Burt2020
spatial null maps.

Map 1 = miR-139-5p-associated left-cortical MSN t map
...
out_dir = (
    PROJECT_ROOT
    / "msn_results_cort"
    / "a5_mir1395p_vs_periph_srpk1_lh_v3"
)
...
result_file = (
    out_dir
    / "a5_mir1395p_vs_srpk1_full_2x4_burt_results_lh_v3.csv"
)

r_matrix_file = (
    out_dir
    / "a5_mir1395p_vs_srpk1_r_obs_matrix_2x4_lh_v3.csv"
)

p_matrix_file = (
    out_dir
    / "a5_mir1395p_vs_srpk1_p_burt_matrix_2x4_lh_v3.csv"
)

meta_file = (
    out_dir
    / "a5_mir1395p_vs_srpk1_full_2x4_meta_lh_v3.json"
)

roi_order_check_file = (
    out_dir / "a5_dk308_lh_roi_order_check_v3.csv"
)
...
def read_roi_order(path):
    ...
    if len(names) != expected_full_n_roi:
        raise ValueError(
            f"Expected {expected_full_n_roi} ROI names in "
...
def prepare_map(
    ...
    if len(df) not in [
        ...
        raise ValueError(
            ...
            f"{expected_full_n_roi} bilateral rows in "
            f"{label}, got {len(df)}."
        )
...
def align_map_to_lh_order(
    ...
    if missing:
        raise ValueError(
            f"LH ROI names missing in {label}: "
            f"{missing[:30]}"
        )
    ...
    if aligned[value_name].isna().any():
        ...
        raise ValueError(
            f"Missing numeric values after LH alignment "
            f"in {label}: {bad[:30]}"
        )

    if len(aligned) != expected_n_roi:
        raise ValueError(
            f"Expected {expected_n_roi} LH ROIs after "
            f"alignment in {label}, got {len(aligned)}."
        )
...
def check_dk308_lh_parcellation(path):
    ...
    if len(labels) != expected_n_roi:
        raise ValueError(
            ...
            f"labels in DK-308 LH parcellation, "
            f"got {len(labels)}."
        )
    ...
    if not np.array_equal(
        ...
        raise ValueError(
            ...
            "parcellated null-map order matches the "
            "LH subset of dk308_roi_order.txt."
        )
...
def generate_or_load_burt_nulls(
    ...
    if cache_is_valid:
        print(
            "Loading cached Map 1-specific LH "
            f"Burt2020 nulls for miRNA model: "
            f"{mirna_model_tag}"
        )
        ...
        if (
            ...
            print(
                "Cached LH Burt2020 nulls do not "
                "match the current Map 1 or settings; "
                f"regenerating for {mirna_model_tag}..."
            )
        else:
            print(
                "Generating Map 1-specific LH "
                f"Burt2020 nulls for miRNA model: "
                f"{mirna_model_tag}"
            )
...
for (
    ...
    for (
        ...
        print(
            "Aligned merged LH ROI count:",
            len(df),
        )
        print(
            "Aligned first LH ROI:",
            df[roi_col].iloc[0],
        )
        print(
            "Aligned last LH ROI:",
        ...
        if len(df) != expected_n_roi:
            raise ValueError(
                f"Expected {expected_n_roi} LH ROIs "
                f"after merge for {mirna_model_tag} x "
        ...
        merged_file = (
            merged_dir
            / f"a5_input_merged_{comparison_tag}_lh_v3.csv"
        )

        null_corr_file = (
            null_dist_dir
            / f"a5_null_distribution_{comparison_tag}_lh_v3.csv"
        )
        ...
        all_results.append({
            ...
            "n_mirna_extra_ignored": len(
                mirna_extra
            ),
            "n_srpk1_extra_ignored": len(
                srpk1_extra_counts[
                    srpk1_model_tag
                ]
            ),
            ...
            "merged_file": str(
                merged_file.relative_to(
                    out_dir
                )
            ),
            "null_corr_file": str(
                null_corr_file.relative_to(
                    out_dir
                )
            ),
        })
...
if len(result_df) != expected_n_comparisons:
    raise ValueError(
        f"Expected {expected_n_comparisons} "
        f"comparisons, got {len(result_df)}."
    )
...
meta = {
    "analysis": (
        "cortical_A5_miR1395p_MSN_tmaps_LH_vs_"
    ...
    "expected_n_rh_in_input_order": (
        expected_n_rh
    ),
    "expected_n_rh_in_analysis": 0,
    "use_zscore": use_zscore,
    "result_file": str(
        result_file
    ),
    ...
    "note": (
        "Map 1 consists of two miR-139-5p-associated "
        "left-cortical MSN t maps: main and plus_med_history. "
        "A separate Map 1-specific Burt2020 null set is generated "
        "for each miRNA model using the 152-label DK-308 "
        "left-hemisphere parcellation in MNI152 2 mm space. "
        "Each null set is reused across the four peripheral SRPK1 "
        "Map 2 models, yielding a complete 2 x 4 matrix of eight "
        "left-cortical spatial comparisons. The miRNA model tags "
        "are main and plus_med_history; they are not labeled as "
