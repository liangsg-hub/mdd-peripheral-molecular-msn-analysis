#!/usr/bin/env python3
"""
Compare left-hemisphere miR-139-5p-associated cortical MSN maps with the MDD-related cortical MSN map using spatial null models.
...
out_dir = PROJECT_ROOT / "msn_results_cort/a1_mir1395p_vs_disease_lh"
out_dir.mkdir(parents=True, exist_ok=True)

result_file = out_dir / "a1_mir1395p_vs_disease_batch_models_burt_results_lh_v1.csv"
meta_file = out_dir / "a1_mir1395p_vs_disease_batch_models_meta_lh_v1.json"
roi_order_check_file = out_dir / "a1_dk308_lh_roi_order_check_v1.csv"
...
def prepare_map(path, value_col, value_name, label):
    ...
    if len(df) not in [expected_full_n_roi, expected_n_roi]:
        raise ValueError(
            f"Expected {expected_full_n_roi} or {expected_n_roi} rows in {label}, got {len(df)}"
        )
...
all_results = []
meta = {
    "analysis": "cortical_A1_miR1395p_MSN_tmap_vs_MDD_HC_MSN_tmap_LH",
    ...
    "note": (
        "The input miR-139-5p and disease maps may contain 308 bilateral DK-308 ROIs. "
        "This script restricts both maps to the 152 left hemisphere ROIs from dk308_roi_order.txt. "
        "The LH vectors are compared using LH DK-308 Burt nulls in the same ROI order."
    ),
}
...
for mirna_model_tag, mirna_map_file in mirna_maps.items():
    ...
    if len(df) != expected_n_roi:
        raise ValueError(f"Expected {expected_n_roi} LH ROIs after merge for {mirna_model_tag}, got {len(df)}")
    ...
    if len(df) != expected_n_roi:
        raise ValueError(
            f"LH ROI count after dropna is not {expected_n_roi} for {mirna_model_tag}."
        )
    ...
    merged_file = out_dir / f"a1_input_merged_maps_mir1395p_{mirna_model_tag}_vs_{disease_model_tag}_aligned_lh_v1.csv"
    null_corr_file = out_dir / f"a1_null_distribution_mir1395p_{mirna_model_tag}_vs_{disease_model_tag}_aligned_lh_v1.csv"
    ...
    all_results.append({
        "comparison": "Peripheral_miR1395p_MSN_tmap_vs_MDD_HC_MSN_tmap_LH",
